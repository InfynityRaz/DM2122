...
{	
	...
	light[0].spotDirection.Set(1.0f, 3.0f, -1.0f);
	...
	camera.Init(Vector3(4, 2, 3), Vector3(0, 2, 0), Vector3(0, 1, 0));
	...
	meshList[GEO_QUAD] = MeshBuilder::GenerateQuad("quad", Color(0, 0.5, 0.5), 50);
	...
	meshList[GEO_BLASTER1] = MeshBuilder::GenerateOBJMTL("blasterA", "OBJ//blasterA.obj", "OBJ//blasterA.mtl");
...
void SceneAssignment2::Update(double dt)
{
	...
	{
		bLButtonState = true;
		std::cout << std::endl << "LBUTTON PRESSED" << std::endl;
	}
	...
	{
		bRButtonState = true;
		std::cout << std::endl << "RBUTTON PRESSED" << std::endl;
	}
...
void SceneAssignment2::Render()
{
	...
	RenderSkybox();
	RenderTextOnScreen(meshList[GEO_TEXT], "FPS:" + std::to_string(FPS), Color(0, 1, 0), 2, 0, 58);

	modelStack.PushMatrix();

	modelStack.Translate(0, 1.5f, 0);
	RenderMesh(meshList[GEO_BLASTER1], true);
